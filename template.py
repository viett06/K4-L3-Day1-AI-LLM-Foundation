...
def call_openai(
    ...
    estimated_time = time.perf_counter() - start

    return response.choices[0].message.content, estimated_time
...
def compare_models(prompt: str) -> dict:
    ...
    open_ai = call_openai(prompt)
    mini_ai = call_openai_mini(prompt)
    cost_open_ai = (len(open_ai[0].split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    cost_open_ai_mini = (len(mini_ai[0].split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o-mini"]["output"]
    return {
        "gpt4o_response": open_ai[0],
        "mini_response": mini_ai[0],
        "gpt4o_latency": open_ai[1],
        "mini_latency": mini_ai[1],
        "gpt4o_cost_estimate": cost_open_ai,
        "mini_cost_estimate": cost_open_ai_mini,
    }
# ===========================================================================
...
def run_assistant(
    ...
    while True:
        ...
        turn_cost = estimate_cost(user_msg, reply)
